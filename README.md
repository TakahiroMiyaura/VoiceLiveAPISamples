...
- **Preview API Features** (2026-01-01-preview):
  - **Foundry Agent Tool**: Chat supervisor pattern where AI Model calls Foundry Agent as a tool
  - **Filler Response**: Automatic filler phrases during latency or tool call wait times (static/LLM-based)
  - **Echo Cancellation**: Server-side echo cancellation to prevent speaker audio from interrupting AI responses

- **Optimized Audio Pipeline**:
  - Large audio buffer (180 seconds) to handle server TTS burst delivery (~2.5x real-time speed)
  - `ReadFully` mode for gapless playback without stop/restart cycles
  - Automatic recording stop on speech end detection
...
| Preview API (2026-01-01) | Foundry Agent Tool, Filler Response | - |
...
// Echo cancellation (prevents speaker audio from triggering server VAD)
options.InputAudioEchoCancellation = new AudioInputEchoCancellationSettings();

// Filler response (2026-01-01-preview API version required)
// Static filler: random selection from predefined phrases
...
// Or LLM-based filler: context-aware responses generated by LLM
options.FillerResponse = new LlmFillerResponseConfig();
```

> [!NOTE]
> Filler Response requires API version `2026-01-01-preview`. Set it via `VoiceLiveClientOptions.ApiVersion`.
> This feature is available only in VoiceLiveConsoleApp (custom WebSocket implementation), not in VoiceLiveSDKConsoleApp.
...
| `OnFoundryAgentCallArgumentsDeltaReceived` | Foundry Agent call arguments streaming (preview) |
| `OnFoundryAgentCallArgumentsDoneReceived` | Foundry Agent call arguments complete (preview) |
| `OnFoundryAgentCallInProgressReceived` | Foundry Agent call in progress (preview) |
| `OnFoundryAgentCallCompletedReceived` | Foundry Agent call completed (preview) |
| `OnFoundryAgentCallFailedReceived` | Foundry Agent call failed (preview) |
