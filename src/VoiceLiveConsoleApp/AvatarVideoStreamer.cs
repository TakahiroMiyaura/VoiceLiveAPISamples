...
namespace Com.Reseul.Azure.AI.Samples.VoiceLiveAPI
{
    ...
    public class AvatarVideoStreamer : IDisposable
    {
        ...
        public AvatarVideoStreamer(AvatarClient avatarClient, ILogger logger)
        {
            ...
            {
                logger?.LogError(ex,
                    "[AvatarVideoStreamer] Failed to initialize Opus decoder - audio will be disabled");
            ...
            try
            {
                ...
                {
                    totalPlaybackStops++;
                    double timeSinceLastFrame = lastAudioFrameReceivedTime != DateTime.MinValue
                    ...
                    {
                        double bufferedMs = waveProvider != null
        ...
        public void StopStreaming()
        {
            ...
            try
            {
                ...
                logger?.LogTrace(
                    "[AvatarVideoStreamer] Stopping streaming. Video frames: {video}, Audio frames: {audio}",
        ...
        private bool StartFFmpegProcess()
        {
            try
            {
                ...
                string ffmpegArgs = $"-re -f h264 -r 25 -probesize 1024 -analyzeduration 1000000 " +
                ...
                if (ffmpegProcess.Start())
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            {
                                string? error = await ffmpegProcess.StandardError.ReadLineAsync();
...
a=fmtp:96 packetization-mode=1";
        ...
        private void StartRealtimeFFplay(int port, string protocol = "rtp")
        {
            try
            {
                ...
                if (protocol == "rtp")
                {
                    ...
                    string sdpFilePath = Path.Combine(Directory.GetCurrentDirectory(), "avatar_stream.sdp");
                    ...
                    if ((DateTime.Now - lastStatusLog).TotalSeconds >= 2)
                    {
                        double bufferedMs = waveProvider != null
                            ? waveProvider.BufferedBytes * 1000.0 /
                              (waveProvider.WaveFormat.SampleRate * waveProvider.WaveFormat.Channels * 2)
                            : 0;
                        string playbackState = waveOut?.PlaybackState.ToString() ?? "null";
                        double timeSinceLastFrame = lastAudioFrameReceivedTime != DateTime.MinValue
                            ? (DateTime.Now - lastAudioFrameReceivedTime).TotalMilliseconds
                            : -1;

                        string syncStatus = initialBufferingComplete ? "Synced" : "Buffering";
                        logger?.LogTrace(
        ...
        private void ProcessSynchronizedFrames()
        {
            ...
            TimeSpan sessionElapsed = DateTime.Now - sessionStartTime.Value;
            ...
            double currentPlaybackTime = (sessionElapsed - initialBufferingDuration).TotalSeconds;
            ...
            int videoFramesProcessed = 0;
            while (videoFrameQueue.TryPeek(out (uint timestamp, byte[] data) videoFrame) &&
                   firstVideoTimestamp.HasValue)
            {
                double videoRelativeTime = (videoFrame.timestamp - firstVideoTimestamp.Value) / VideoClockRate;
            ...
            int audioFramesProcessed = 0;
            while (audioFrameQueue.TryPeek(out (uint timestamp, byte[] pcmData, double dbLevel) audioFrame) &&
                   firstAudioTimestamp.HasValue)
            {
                double audioRelativeTime = (audioFrame.timestamp - firstAudioTimestamp.Value) / AudioClockRate;
            ...
            if (audioFrameQueue.Count > 50 && audioFramesProcessed == 0 && initialBufferingComplete)
            {
                ...
                if (audioFrameQueue.TryPeek(out (uint timestamp, byte[] pcmData, double dbLevel) nextFrame))
                {
                    if (firstAudioTimestamp != null)
                    {
                        double nextRelativeTime = (nextFrame.timestamp - firstAudioTimestamp.Value) / AudioClockRate;
                        logger?.LogWarning(
        ...
        private static bool HasNALUnit(byte[] data, byte nalType)
        {
            for (int i = 0; i < data.Length - 4; i++)
            {
                ...
                if (data[i] == 0x00 && data[i + 1] == 0x00)
                {
                    ...
                    {
                        int nalUnitType = data[nalStart] & 0x1F;
        ...
        private void ProcessVideoFrame(byte[] frameData)
        {
            try
            {
                ...
                bool hasSPS = HasNALUnit(frameData, 7); // SPS (0x67 & 0x1F = 7)
                bool hasPPS = HasNALUnit(frameData, 8); // PPS (0x68 & 0x1F = 8)
                bool hasIDR = HasNALUnit(frameData, 5); // IDR frame (0x65 & 0x1F = 5)
        ...
        private void OnVideoFrame(IPEndPoint remote, uint ssrc, byte[] frame, VideoFormat fmt, uint timestamp)
        {
            ...
            try
            {
                ...
                byte[] reconstructedFrame = streamReconstructor?.ProcessFrame(frame) ?? [];
                ...
                {
                    double relativeTime = (timestamp - firstVideoTimestamp.Value) / VideoClockRate;
                    logger?.LogTrace(
        ...
        private void OnAudioFrame(byte[] audioData, uint timestamp)
        {
            ...
            try
            {
                ...
                double timeSinceLastFrame = previousAudioFrameReceivedTime != DateTime.MinValue
                ...
                if (opusDecoder != null)
                {
                    try
                    {
                        ...
                        if (decodedSamples > 0)
                        {
                            ...
                            byte[] pcmBytes = new byte[decodedSamples * 2 * 2]; // stereo, 16-bit
                            ...
                            double rms = Math.Sqrt(sumSquares / (decodedSamples * 2));
                            double dbLevel = 20 * Math.Log10(rms / 32768.0); // Convert to dB
                            bool isSilent = dbLevel < -60;
                            ...
                            else
                            {
                                ...
                                else if (waveProvider != null)
                                {
                                    ...
                                    double bufferedMs = waveProvider.BufferedBytes * 1000.0 /
