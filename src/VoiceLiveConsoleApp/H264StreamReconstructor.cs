    ...
    public class H264StreamReconstructor
    {
        ...
        public byte[] ProcessFrame(byte[] frameData)
        {
            ...
            try
            {
                ...
                List<NalUnit> nalUnits = ParseNalUnits(frameData);

                // Cache SPS/PPS if found
                CacheSpsAndPps(nalUnits);

                // Check if we need to inject headers
                bool needsHeaderInjection = ShouldInjectHeaders(nalUnits);
            ...
            for (int i = 0; i < data.Length - 4; i++)
            {
                ...
                if (isStartCode)
                {
                    ...
                    if (nalHeaderOffset < data.Length)
                    {
                        ...
                        int totalSize = startCodeLength + nalSize;
                        byte[] nalData = new byte[totalSize];
        ...
        {
            NalUnit? sps = nalUnits.FirstOrDefault(n => n.Type == 7);
            NalUnit? pps = nalUnits.FirstOrDefault(n => n.Type == 8);
        ...
        private bool ShouldInjectHeaders(List<NalUnit> nalUnits)
        {
            ...
            bool hasIDR = nalUnits.Any(n => n.Type == 5);
            if (hasIDR)
                return true;

            // Inject periodically (every N frames)
            bool isPeriodicFrame = frameCount % InjectionInterval == 0;
            if (isPeriodicFrame)
                return true;

            // Don't inject if frame already has SPS/PPS
            bool hasSPS = nalUnits.Any(n => n.Type == 7);
            bool hasPPS = nalUnits.Any(n => n.Type == 8);
            if (hasSPS || hasPPS)
                return false;
        ...
        private byte[] CreateFrameWithHeaders(byte[] originalFrame, List<NalUnit> nalUnits)
        {
            ...
            int totalSize = cachedSPS.Length + cachedPPS.Length + originalFrame.Length;
            byte[] reconstructedFrame = new byte[totalSize];
