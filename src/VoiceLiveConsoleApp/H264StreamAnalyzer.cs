...
public class H264StreamAnalyzer
{
    ...
    public H264StreamAnalyzer(ILogger logger)
    {
        ...
        var analysisPath = $"h264_analysis_{timestamp}.txt";
    ...
    public void AnalyzeFrame(byte[] frameData)
    {
        ...
        try
        {
            ...
            // Analyze NAL units
            var nalUnits = FindNalUnits(frameData);
            analysis.AppendLine($"NAL Units found: {nalUnits.Count}");

            foreach (var nal in nalUnits)
            {
                var nalType = nal.type;
                var nalTypeName = GetNalTypeName(nalType);
                analysis.AppendLine(
            ...
            var hasSPS = nalUnits.Exists(n => n.type == 7);
            var hasPPS = nalUnits.Exists(n => n.type == 8);
            var hasIDR = nalUnits.Exists(n => n.type == 5);
            var hasSlice = nalUnits.Exists(n => n.type == 1);
            ...
            var analysisBytes = Encoding.UTF8.GetBytes(analysis.ToString());
    ...
    {
        var nalUnits = new List<(int type, int offset, int size)>();
